...
# Initial seed number. The first simulation (sim_0) will have this seed. Then all subsequent ones will increase the seed by 1.
initial_seed = 42
...
for duration in durations:
    for param_index, params in enumerate(parameter_combinations):
        ...
        for simulation_num in range(1, num_simulations + 1):
            ...
            file_name = f"sim_{name_prefix}_seeded_neuron_time_matrix_{duration}secs_r={int(r * 1000)}_{'yes' if using_sine_wave else 'no'}Sine"
            directory_path = os.path.join(r"../SimulatedDataSets/multipleDatasetsSeeded", f"seeded{duration}SecondData", param_identifier)
            ...
            for n in range(neuron_num):
                np.random.seed(initial_seed + simulation_num * 100 + n)  # Seed for reproducibility.
                print(params, ",", n)
                counter_poisson = 0
                counter = 0
