...
dataSeries = 20; %Which dataseries (time durations) we are working with


%the combination of parameters being tested
%currently, the 4 parameter combinations are:
%'r=30_noSine', 'r=30_yesSine', 'r=60_noSine', 'r=60_yesSine'
parameter_combo = 'r=30_noSine';

%change to whatever folder the data is stored in.
%to change from seeded to unseeded data, change the "multipleDatasetsSeeded"
%to "multipleDatasetsUnseeded" and the 'seeded%dSecondData' to
%'unseeded%dSecondData'
rawpath = sprintf('\\\\middcloud.middlebury.edu\\middfiles\\SpecialProjects\\DashResearch\\InfraslowProject\\MaxBluhmSpring2024\\SimulatedDataSets\\multipleDatasetsSeeded\\seeded%dSecondData\\%s', dataSeries, parameter_combo);
...
figpath = 'C:\MATLAB\multiple';
outputpath = 'C:\MATLAB\multiple';
...
% Print out available files
disp('Available files:');
for i = 1:numel(nameArray)
    disp([num2str(i), ': ', nameArray{i}]);
end

% Prompt user to enter an integer corresponding to the file to run
fileIndex = input('Enter the index of the file you want to run: ');

if fileIndex >= 1 && fileIndex <= numel(nameArray)
    % Get the filename corresponding to the entered index
    selectedFilename = fullfile(rawpath, nameArray{fileIndex});
else
    disp('Invalid file index.');
end

nameArray(fileIndex)
%clearvars -except BinSizes MaxLags rawpath nameArray
[a, b, c] = xlsread(selectedFilename);
spM2 = a;
nneu = size(spM2, 1);
